...
async function webtoons(title, episode, progress = false) {
  /*
  ...
  for (let i = 0; i < height; i += 10000) {
    ...
    await page.waitFor(1000); // Awaiting render after scrolldown action
...
async function batchDownload(webtoonId, start, end) {
  for(let i = start; i <= end; ++i) {
    await webtoons(webtoonId, i);
  }
}
